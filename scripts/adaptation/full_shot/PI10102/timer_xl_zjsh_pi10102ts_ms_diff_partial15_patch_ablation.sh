...
# with partial-loss fine-tuning (loss applied only to the first 15 prediction
# steps per token). Runs patch_num in PATCH_NUMS (default: 1 3 5). Names
# include _partial15 and _p${patch_num}, so checkpoints/results never overwrite
# the 8-patch run or the full-loss p1/p3/p5 ablation.
...
for token_num in $PATCH_NUMS; do
  ...
  echo "============================================"
  echo "Timer-XL MS DIFF partial15 patch ablation: p${token_num}, seq_len=${seq_len}"
  echo "============================================"
  "$PYTHON_BIN" -u run_partial.py \
    --task_name forecast \
    --is_training 1 \
    --root_path "$root_path" \
    --data_path splits.yaml \
    --split_file "$split_file" \
    --model_id "ZJSH_PI10102TS_MS_partial15_DIFF_p${token_num}" \
    --model "$model_name" \
