#!/usr/bin/env bash
source "$(dirname "$0")/_common_diff.sh"

python -u run.py \
  --task_name long_term_forecast --is_training 1 \
  --root_path "$ROOT_PATH" --split_file "$SPLIT_FILE" \
  --data MultivariateDatasetYAMLSplit \
  --model_id ZJSH_PI10102TS_DiPCALSTM_MS_DIFF --model DiPCALSTM \
